...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def camera_capture(self):
        ...
        try:
            ...
            if os.path.exists(new_dir):
                self.textEdit_message.setText(str(new_message)+"Picture Captured and Stored in‣ \n" + pic_dir) 
                QMessageBox.information(None,"Capture successful",str(new_message)+"Picture Captured and Stored in‣ \n" + pic_dir) 
                 
            else:
                self.textEdit_message.setText("Image not Captured")
                QMessageBox.critical(None,"Image not captured","Image not Captured‣\n➊Turn off the camera and turn it back on.\n➋Check USB cable")            

        except:
            self.textEdit_message.setText("➊Check if camera is powered on.\n➋Remove lens cover.\n➌Turn off the camera and turn it back on.\n➍Replace Battery.\n➎Place an object in front of the camera.")
            QMessageBox.critical(None,"Lens Error","➊Check if camera is powered on.\n➋Remove lens cover.\n➌Turn off the camera and turn it back on.\n➍Replace Battery.\n➎Place an object in front of the camera.")           
    ...
    def gui_setup(self):
        ...
        if self.email =="" or self.email == "Email1, Email2 ...":
            ...
            email_flag = 0
            receiver_email = [x.strip() for x in str(self.ui.lineEdit_email.text()).split(",")]
        ...
        if self.foodload == "Other":
            ...
            if foodflag == 0:        
                for i in range(0,len(self.comboBox_foodload)):
                    if self.ui.comboBox_foodload.itemText(i).lower() == self.foodload.lower():
                        foodcount += 1
        ...
        if flag ==0 and foodflag == 0:
            ...
            pic_dir = name_dir + "/" + str(self.foodload) + " " + str(time.strftime("%m.%d.%Y"))
    ...
    def liveView(self):
        ...
        try:
            self.textEdit_message.setText("If LiveView is not visible:\n‣Check if camera is powered on.\n‣Unplug and replug the Camera USB.")
            if not os.path.exists(fifofile_dir):
                time.sleep(1)
                process1 = subprocess.Popen(makefile, shell = True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,close_fds=False)   
            
            process = subprocess.Popen(bashCommand, shell = True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,close_fds=False)
            try:
                output, error = process.communicate()
            except:
                self.textEdit_message.setText("‣Check Camera Connection.\n➊Close the application.\n➋Unplug and re plug the camera USB cable.")
                QMessageBox.critical(None,"LiveView Error","‣Check Camera Connection.\n➊Close the application.\n➋Unplug and re plug the camera USB cable.")                        
    ...
    def camerasummary(self):
        ...
        try:
            ...
            self.textEdit_message.setText("Camera connected successfully\n‣Click Away")
            ...
            if int(battery)==0:
                ...
                QMessageBox.warning(None,"Battery info","Recharge the Battery")
            ...
            self.textEdit_camstatus.setText("Camera Summary\n==============\n" + str(text))
            ...
            self.textEdit_message.setText("No Camera detected - Check the following:\n➊Camera is ON.\n➋Battery is present and charged.\n➌USB is connected.")
            QMessageBox.warning(None,"No Camera detected","Check the following:\n➊Camera is ON.\n➋Battery is present and charged.\n➌USB is connected.")
    ...
    def SendMail(self):
        ...
        msg = MIMEMultipart()
        msg['Subject'] = "Photobox Image: " + str(self.foodload) + " " + str(time.strftime("%m.%d.%Y")) 
        msg['From'] = 'sharkninjatestfixtures@gmail.com'
        # ~ msg['To'] = receiver_email

        text = MIMEText("Name: " + str(self.name) + "\n" 
        +"Foodload: " + str(self.foodload) + "\n"
        +"Time: " + str(time.strftime("%r")) + "\n"
        +"Date: " + str(time.strftime("%m.%d.%Y (%A)")) + "\n\n"
        +"Regards,\nNinja Test Team")
        ...
        for i in range(0,len(receiver_email)):
            ...
            if is_valid and is_valid2 == True:
                smtpserver.sendmail(gmail_user, receiver_email[i], msg.as_string())
